...
// ============================================================
//  Утилита: экранирование строк для JSON (поддержка кириллицы)
// ============================================================
...
// ============================================================
//  Оригинальные классы — логика не изменена
// ============================================================
...
public:
    std::string froth_milk(int ml, Storage& s) {
        if (ml > 0) {
            s.consume_milk(ml);
            return "Капучинатор: взбиваем " + std::to_string(ml) + "мл молока";
        }
        return "";
    }
};
...
public:
    std::string dispense_hot_water(int ml, Storage& s) {
        if (ml > 0) {
            s.consume_water(ml);
            return "Бойлер: горячая вода " + std::to_string(ml) + "мл";
        }
        return "";
    }
};
...
public:
    std::string add_sugar(int g, Storage& s) {
        if (g > 0) {
            s.consume_sugar(g);
            return "Дозатор: сахар " + std::to_string(g) + "г";
        }
        return "";
    }
};
...
// ============================================================
//  HTTP сервер — единственный новый код (~50 строк)
// ============================================================
...
int main() {
    ...
    // Разрешаем CORS — чтобы браузер мог обращаться к серверу
    auto set_cors = [](httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        res.set_header("Content-Type", "application/json; charset=utf-8");
    };

    // GET /storage — текущее состояние резервуаров
    ...
    // Тело запроса: {"type":"cappuccino","sugar":2}
    server.Post("/brew", [&](const httplib::Request& req, httplib::Response& res) {
        set_cors(res);

        // Простой парсинг JSON вручную (без внешних библиотек)
    ...
    // OPTIONS — preflight запросы браузера (нужно для CORS)
    ...
    std::cout << "CafeOS server running at http://localhost:8080" << std::endl;
    std::cout << "Open index.html in browser to use the UI" << std::endl;
